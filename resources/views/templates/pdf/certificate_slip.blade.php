...
<div class="mt-4">
    <div class="mb-4">
        Rechnung an: {{ $certificate->name }}<br>
        {{$certificate->street}} {{$certificate->house_number}}<br>
        {{$certificate->postcode}} {{$certificate->city}}<br><br>
        <strong>Rechnung Nr.: {{ $certificate->certificate_number }}</strong>
    </div>
    ...
    <table class="table-border w-100p font-size-9">
        ...
        <tbody>
        @foreach($certificate->services as $index=>$service)
            <tr>
        ...
        <tr>
            ...
            <td class="text-end pe-2 p-1 fw-bold">{{ formatNumber($certificate->getTotalNetPrice()) }} €</td>
        </tr>
        </tbody>
    </table>
    ...
    <div class="mt-4 float-start" style="width: 250px;">
        <table class="w-100p border-0 font-size-10">
            <tr>
                <td>Netto:</td>
                <td class="text-end">{{ formatNumber($certificate->getTotalNetPrice()) }} €</td>
            </tr>
            <tr>
                <td class="fw-bold">MwSt (19%):</td>
                <td class="text-end">{{ formatNumber($certificate->getTaxPrice()) }} €</td>
            </tr>
            <tr class="border-top-0">
                <td class="fw-bold">Rechnungsbetrag:</td>
                <td class="text-end fw-bold">{{ formatNumber($certificate->getTotalGrossPrice()) }} €</td>
            </tr>
        </table>
    </div>
